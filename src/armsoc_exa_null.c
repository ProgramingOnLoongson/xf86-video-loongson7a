...
struct ARMSOCEXARec * InitNullEXA(ScreenPtr pScreen, ScrnInfoPtr pScrn, int fd)
{
    ...
    // 32
    pExaDrv->pixmapPitchAlign = 32 * 4;
    pExaDrv->flags = EXA_OFFSCREEN_PIXMAPS | EXA_HANDLES_PIXMAPS | EXA_SUPPORTS_PREPARE_AUX;
    pExaDrv->maxX = 2048;
    pExaDrv->maxY = 2048;
    ...
    pExaDrv->ModifyPixmapHeader = LS_ModifyPixmapHeader;

    pExaDrv->PrepareAccess = ARMSOCPrepareAccess;
    pExaDrv->FinishAccess = ARMSOCFinishAccess;
    pExaDrv->PixmapIsOffscreen = IsPixmapOffscreen;
